...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct 20 13:44:54 2021
...
path = "/Users/brunomorgado/Dropbox/mac/Mac_Education/Centennial_College/Third_semester/AI/Assignments/Assignment3/"
filename = 'Ecom Expense.csv'
fullpath = os.path.join(path,filename)

#Storing the data into a variable
ecom_exp_bruno = pd.read_csv(fullpath)


#Initial exploration
ecom_exp_bruno.head(3)
ecom_exp_bruno.tail()
ecom_exp_bruno.shape
ecom_exp_bruno.columns
ecom_exp_bruno.dtypes
ecom_exp_bruno.isna().sum()
ecom_exp_bruno.info()


#Removing 'Transaction ID' column for it is only a sequential index. Therefore, of little value for our analysis.
ecom_exp_bruno.drop('Transaction ID', axis=1, inplace=True)

#Confirming the removal of the Transaction ID Column
ecom_exp_bruno.head()

#Confirming the number of categories in Gender and City Tier 
ecom_exp_bruno.Gender.value_counts()
ecom_exp_bruno["City Tier"].value_counts()

#Transforming categorical variables into numerical variables
gender_dummies = pd.get_dummies(ecom_exp_bruno.Gender, prefix="Sex")
cityTier_dummies= pd.get_dummies(ecom_exp_bruno["City Tier"], prefix="City")

#Concatenating the newly created dummy variables to the base dataframe
df_ecom = pd.concat([ecom_exp_bruno, gender_dummies, cityTier_dummies], axis = 1)
...
X_train_bruno, X_test_bruno, y_train_bruno, y_test_bruno = train_test_split(df_x, df_y, test_size=0.35, random_state=98)

#Instantiating a Linear model
model = LinearRegression()

#Fitting the model onto the training data
model.fit(X_train_bruno, y_train_bruno)
...
model.score(X_test_bruno, y_test_bruno)

#Predicting the target variable by using the test predictors
y_prediction = model.predict(X_test_bruno)
...
X_train_bruno2, X_test_bruno2, y_train_bruno2, y_test_bruno2 = train_test_split(df_x2, df_y, test_size=0.35, random_state=98)

#Fitting the model onto the new data
model.fit(X_train_bruno2, y_train_bruno2)
...
#R^2
model.score(X_test_bruno2, y_test_bruno2)
...
df_train = pd.concat([X_train_bruno, y_train_bruno], axis = 1)
...
df_train_record = pd.concat([X_train_bruno2, y_train_bruno2], axis = 1)
